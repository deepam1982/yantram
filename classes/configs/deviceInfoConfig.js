...
var DevInfoConfigManager = BasicConfigManager.extend({
	...
	getList : function () {
		...
		var maxUpdateTs = Math.max(ipCamaraConfig.updateTs, irBlasterConfig.updateTs, irRemoteConfig.updateTs)
		if(!this.stableData.length || this.stableDataUpdateTs < maxUpdateTs) {
			this.stableDataUpdateTs = maxUpdateTs;
			this.stableData = [];
			...
			this.stableData.push(irBlstrData);
			...
			this.stableData.push(irRemData);
		}
		return data.concat(this.stableData);
	},
	...
	getLoadInfo : function (deviceId, loadId) {
		...
		if(!this.loadInfoMap[loadIdForMap] || this.loadInfoMap[loadIdForMap].ts < maxUpdateTs){
			...
			this.loadInfoMap[loadIdForMap].ctl = ctl
			this.loadInfoMap[loadIdForMap].sensorIndx = sensorIndx;
